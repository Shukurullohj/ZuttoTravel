...
@section('content')
 <section class="p-5 bg-gradient-to-r from-green-200 to-white m-0">
    <div class="flex justify-between">
        ...
        <div class="flex">
            <button class="border-4 border-yellow-600 rounded px-3">{{$place->perfecture->name}}</button>
    ...
    <div class="mt-4">
        ...
        <a class="block text-right text-primary text-xl font-bold" href=""></a>
    </div>
     <form action="{{$place->path() . '/comments' }}" method="POST">
         @csrf
         <div class="my-4">          
             <textarea class=" rounded bg-green-100" name="comment" id="comment" cols="30" rows="7" placeholder="Add your comments"></textarea>
             <button class="border-4 border-purple-500 rounded">Submit</button>
         </div>
     </form>
      @foreach ($place->comments as $comment)

      <div class="border-4 border-green-500 p-3"><p>{{$comment->comment}}</p></div>
      <div><p class="text-right">{{$place->creator->name}}<span class="ml-3">{{$comment->created_at->diffForHumans()}}...</span></p>
        </div>
